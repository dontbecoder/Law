...
# 股东代表诉讼 = 派生诉讼 = 股东代位诉讼
	if ('公司合法权益受到不法侵害')and ('公司怠于起诉'):
		('股东以自己名义其实')
		
	if ('股东代表诉讼'):
		if ('有限责任公司的股东') or ('连续180日以上单独或合计持股1%以上的股东'):
			if ('董事、高管违法') and  ('有监事会'or'有监事'):
				('请求监事会或监事向法院提起诉讼')
			if ('监事违法') and ('有董事会'or'有董事'):
				('请求董事会或董事向法院提起诉讼')
...
# 股份转让与回购
	...
	if ('股东向股东以外的人转让股份'):
		if ('有限公司'):
			('应经其他股东过半数同意'and'书面通知其他股东') # 其他股东自接收通知之日起满30日未答复的，视为同意；不同意又不优先购买的，视为同意
		elif ('股份公司'):
			('自由转让')
	...
	if ('发起人'):
		...
		if ('股份公司'):
			('公司公开发行前已发行的股份，自公司股票在证券交易所上市交易之日起1年内不得转让')
	
	if ('董事') or ('监事') or ('高管'):
		('向公司申报所持本公司的股份及其变动情况')
		('在任职期间，每年转让的股份不得超过其所持本公司股份的25%')
		('所持本公司股份，自公司股票在证券交易所上市交易之日起1年内不得转让')
...
# 股东抽逃出资
	if ('制作虚假财务会计报表虚增利润进行分配' or '通过虚构债权债务关系将其出资转出' or '利用关联交易将出资转出' or '其他未经法定程序将出资抽回的行为'):
			('构成抽逃出资')
